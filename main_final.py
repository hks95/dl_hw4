...
def train(args, listener_model, train_loader,optimizer, epoch,gpu):
    # listener_model.train()
    epoch_loss = 0
    correct = 0

    # for batch_idx, (data, target,data_lengths,target_lengths) in enumerate(train_loader):
    for batch_idx,(data,data_lengths) in enumerate(train_loader):
        #pdb.set_trace()
        ...
        data = Variable(data)
        data_lengths = Variable(data_lengths, requires_grad=False)
        ...
        optimizer.zero_grad()
        listener_features = listener_model(data,data_lengths) #comes out at float


        # target_ctc = np.concatenate(target)
        # target_ctc = Variable(torch.from_numpy(target_ctc).int())
        # target_lengths = Variable(torch.from_numpy(target_lengths).int(),requires_grad=False)
        #
        # batch_loss_norm = ctc(logits, target_ctc,data_lengths.cpu(), target_lengths.cpu())
        #
        # batch_loss_norm.backward()
        # torch.nn.utils.clip_grad_norm_(model.parameters(), 0.20)
        # optimizer.step()
        # epoch_loss += batch_loss_norm.item()
        #
        # if batch_idx % args.log_interval == 0:
        #     print('Train Epoch: {} \tbatch {} \tLoss: {:.6f}'.format(epoch,batch_idx,batch_loss_norm.item()))
...
def main():
    ...
    listener_model = listener.listenerModel(40,256,embed_drop=0,lock_dropi=0.0,lock_droph=0,lock_dropo=0.0)
    if gpu is True:
    ...
    if args.eval is False:
        ...
        params = {'batch_size': args.batch_size,'num_workers': args.workers, 'shuffle': True,'collate_fn':data_loader.collate } # if use_cuda else {}
        train_loader = data.DataLoader(training_set, **params)

        validation_set = ctc_Dataset('dev', batch_size=args.test_batch_size)
        params = {'batch_size': args.test_batch_size,'num_workers': args.workers, 'shuffle': False,'collate_fn':data_loader.collate }
        ...
        for epoch in range(1,2):
            train(args, listener_model, train_loader,optimizer, epoch,gpu)
