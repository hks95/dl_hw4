...
class SpellerModel(nn.Module):
    ...
    def forward(self, target, target_lengths, attention_key, attention_val, attention_mask,gpu):
        ...
        output_list = []
        output_list.append(torch.zeros(target.shape[1],self.vocab_size).cuda())
        ...
        for i in range(target.shape[0]-1): # batch * 1 word
            ...
            # if prob>=90:
            #     dist = Gumbel(0, 1)
            #     eps = dist.sample(prev_output.size()).cuda()
